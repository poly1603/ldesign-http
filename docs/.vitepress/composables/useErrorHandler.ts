...
return
    ...
    if (errorLog.retryCount >= maxRetries)
return
    ...
    // 计算延迟时间（指数退避）
    const delay = (errorConfig.retryDelay || 1000) * 2 ** (errorLog.retryCount - 1)
...
// 设置全局错误配置
export function setGlobalErrorConfig(config: Partial<ErrorConfig>): void {
