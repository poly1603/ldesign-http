...
return '1'
      if (bodyType.value === 'form' && formData.value.some(f => f.key && f.value))
return '1'
      if (bodyType.value === 'file' && uploadedFiles.value.length)
return uploadedFiles.value.length
      if (bodyType.value === 'raw' && rawText.value.trim())
return '1'
      return null
    default:
      return null
...
function downloadResponse(data, filename = 'response.json') {
          ...
          <div class="param-list">
            <div v-for="(param, index) in queryParams" :key="index" class="param-row">
              ...
              <input
                ...
                @input="updateParamCount"
              >
              <button class="remove-btn" title="删除" @click="removeParam(index)">
                ×
              </button>
            </div>
          </div>
          ...
          <div class="header-list">
            <div v-for="(header, index) in requestHeaders" :key="index" class="param-row">
              ...
              <input
                ...
                @input="updateHeaderCount"
              >
              <button class="remove-btn" title="删除" @click="removeHeader(index)">
                ×
              </button>
            </div>
          </div>
        ...
        <div v-if="activeTab === 'body'" class="body-section">
          ...
          <div v-if="bodyType === 'json'" class="json-body">
            <div class="json-toolbar">
              ...
              <button class="format-btn" @click="minifyJson">
                压缩
              </button>
              <button class="format-btn" @click="clearBody">
                清空
              </button>
            </div>
            <textarea
          ...
          <div v-if="bodyType === 'form'" class="form-data">
            ...
            <div v-for="(field, index) in formData" :key="index" class="param-row">
              ...
              <input
                ...
                class="param-input"
              >
              <button class="remove-btn" title="删除" @click="removeFormField(index)">
                ×
              </button>
            </div>
          </div>
        ...
        <div v-if="activeTab === 'config'" class="config-section">
          ...
          <div class="config-grid">
            ...
            <div class="config-item checkbox-item">
              <label>
                <input v-model="requestConfig.withCredentials" type="checkbox">
