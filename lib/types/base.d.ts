 ...
 * 响应类型
 */
export type ResponseType = 'json' | 'text' | 'blob' | 'arrayBuffer' | 'formData' | 'stream' | 'document';
/**
 * 请求优先级
 */
 ...
 * 创建类型化配置
 */
export declare function createTypedConfig<T = any>(config: RequestConfig): RequestConfig & {
    _type?: T;
};
/**
 * 判断是否为HTTP错误
 ...
 * 判断是否为客户端错误
 */
export declare function isClientError(error: any): boolean;
/**
 * 判断是否为服务器错误
 */
