...
export interface TypedHttpClient<TBaseResponse = unknown> extends Omit<HttpClient, 'upload' | 'download'> {
    ...
    post: <T = TBaseResponse, D = unknown>(url: string, data?: D, config?: RequestConfig) => Promise<ResponseData<T>>;
    put: <T = TBaseResponse, D = unknown>(url: string, data?: D, config?: RequestConfig) => Promise<ResponseData<T>>;
    delete: <T = TBaseResponse>(url: string, config?: RequestConfig) => Promise<ResponseData<T>>;
    patch: <T = TBaseResponse, D = unknown>(url: string, data?: D, config?: RequestConfig) => Promise<ResponseData<T>>;
    ...
    /** 上传文件 */
    upload?: <T = TBaseResponse>(url: string, file: File | File[], config?: UploadConfig) => Promise<UploadResult<T>>;
    /** 下载文件 */
    download?: (url: string, config?: DownloadConfig) => Promise<DownloadResult>;
}
/**
 ...
 * 类型化的请求配置
 */
export interface TypedRequestConfig<TData = unknown> extends Omit<RequestConfig, 'data'> {
    data?: TData;
}
/**
 * 类型化的响应数据
 */
export interface TypedResponseData<TData = unknown> extends Omit<ResponseData, 'data'> {
    data: TData;
}
/**
 * 状态码类型
...
export interface TypedHttpMethods {
    /** GET 请求 */
    get: <TResponse = unknown, TParams = unknown>(url: string, config?: StrictRequestConfig<never, TParams>) => Promise<StrictResponseData<TResponse>>;
    /** POST 请求 */
    post: <TResponse = unknown, TData = unknown, TParams = unknown>(url: string, data?: TData, config?: StrictRequestConfig<TData, TParams>) => Promise<StrictResponseData<TResponse>>;
    /** PUT 请求 */
    put: <TResponse = unknown, TData = unknown, TParams = unknown>(url: string, data?: TData, config?: StrictRequestConfig<TData, TParams>) => Promise<StrictResponseData<TResponse>>;
    /** PATCH 请求 */
    patch: <TResponse = unknown, TData = unknown, TParams = unknown>(url: string, data?: TData, config?: StrictRequestConfig<TData, TParams>) => Promise<StrictResponseData<TResponse>>;
    /** DELETE 请求 */
    delete: <TResponse = unknown, TParams = unknown>(url: string, config?: StrictRequestConfig<never, TParams>) => Promise<StrictResponseData<TResponse>>;
    /** HEAD 请求 */
    head: <TParams = unknown>(url: string, config?: StrictRequestConfig<never, TParams>) => Promise<StrictResponseData<never>>;
    /** OPTIONS 请求 */
    options: <TResponse = unknown, TParams = unknown>(url: string, config?: StrictRequestConfig<never, TParams>) => Promise<StrictResponseData<TResponse>>;
}
/**
 * 条件类型：根据方法确定数据类型
 */
export type RequestDataByMethod<TMethod extends HttpMethod, TData = unknown> = TMethod extends 'GET' | 'HEAD' | 'DELETE' ? never : TData;
/**
 * 条件类型：根据响应类型确定数据类型
 */
export type ResponseDataByType<TResponseType extends RequestConfig['responseType'], TData = unknown> = TResponseType extends 'json' ? TData : TResponseType extends 'text' ? string : TResponseType extends 'blob' ? Blob : TResponseType extends 'arrayBuffer' ? ArrayBuffer : TResponseType extends 'stream' ? ReadableStream : TData;
/**
 * 工具类型：提取 Promise 的类型
 */
 ...
 * 类型守卫：检查是否为网络错误
 */
export declare function isNetworkError(error: unknown): error is HttpError & {
    isNetworkError: true;
};
/**
 * 类型守卫：检查是否为超时错误
 */
export declare function isTimeoutError(error: unknown): error is HttpError & {
    isTimeoutError: true;
};
/**
 * 类型守卫：检查是否为取消错误
 */
export declare function isCancelError(error: unknown): error is HttpError & {
    isCancelError: true;
};
/**
 * 类型守卫：检查响应状态码
